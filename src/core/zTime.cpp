 ...
 * \brief 是否同年
 */
bool zTimeTool::isSameYear(const time_t& t1, const time_t& t2)
{
	zTime time1(t1);
	zTime time2(t2);
 ...
 * \brief 是否同月
 */
bool zTimeTool::isSameMonth(const time_t& t1, const time_t& t2)
{
	zTime time1(t1);
	zTime time2(t2);
	return time1.getYear() == time2.getYear() && time1.getMonth() == time2.getMonth();
}

/**
 * \brief 是否同周
 */
bool zTimeTool::isSameWeek(const time_t& t1, const time_t& t2)
{
	return getWeekStartTime(t1) == getWeekStartTime(t2);
}

/**
 * \brief 是否同天
 */
bool zTimeTool::isSameDay(const time_t& t1, const time_t& t2)
{
	zTime time1(t1);
	zTime time2(t2);
	return time1.getYear() == time2.getYear() && time1.getYDay() == time2.getYDay();
}

/**
* \brief 获取一天起始时间
*/
time_t zTimeTool::getDayStartTime(const time_t& t)
{
	zTime time(t);
	time_t secs = time.getHour()*60*60 + time.getMin()*60 + time.getSec();
...
* \brief 获取一周起始时间
*/
time_t zTimeTool::getWeekStartTime(const time_t& t)
{
	zTime time(t);
	int wDay = (time.getWDay() + 6) % 7;

	return getDayStartTime(t) - wDay*24*60*60;
}
...
FunctionTimesV2::~FunctionTimesV2()
{
	if (2 == Fir::function_times)
	{
		...
		{
			Fir::logger->debug("[函数统计V2]:执行次数:%llu,执行总时间:%llu 微秒,SQL操作:%s,地址:%llu", iter->second->_times, iter->second->_total_time,iter->second->_sqlOP, iter->first);
