...
class zTCPTask : public zProcessor, private zNoncopyable
{
	...
	public:
		...
		zTCPTask(
				...
				const bool checkReq = false) 
				: mSocket(sock, addr, compress)
				, pool(pool)
				, lifeTime()
				, _checkSignal(checkSignal)
				, _tick_timer(10)
				, tick(false)
				, _checkReq(checkReq)
		{
		...
		bool send_msg( Packet& pkt )
		{
			Packet newP;
			newP.resize(pkt.size());
			newP << pkt;

			sendCmd((const void*)&newP[0],newP.getPosition());
			return true;
		}

		bool send_msg( WORD msgID )
		{
			Packet p;
		...
		template<typename T1>
			bool send_msg( WORD msgID, T1& p1 )
			{
				Packet p;
		...
		template<typename T1, typename T2>
			bool send_msg( WORD msgID, T1& p1, T2& p2 )
			{
				Packet p;
				p << msgID;
				p << p1;
		...
		template<typename T1, typename T2, typename T3>
			bool send_msg( WORD msgID, T1& p1, T2& p2, T3& p3 )
			{
				Packet p;
				p << msgID;
				p << p1;
		...
		template<typename T1, typename T2, typename T3, typename T4>
			bool send_msg( WORD msgID, T1& p1, T2& p2, T3& p3, T4& p4 )
			{
				Packet p;
				p << msgID;
				p << p1;
		...
		template<typename T1, typename T2, typename T3, typename T4, typename T5>
			bool send_msg( WORD msgID, T1& p1, T2& p2, T3& p3, T4& p4, T5& p5 )
			{
				Packet p;
				p << msgID;
				p << p1;
		...
		bool send_msg( WORD msgID, Packet& p )
		{
			Packet newP;
			WORD newSize = p.size() + sizeof(WORD);
			newP.resize(newSize);
			newP << msgID;
			newP << p;
			sendCmd((const void*)&newP[0],newP.getPosition());
		 ...
		 *
		 */
		unsigned char getReq() { return _req; }
		/**
		 * \brief 是否检测请求序号
		 * \return true or false
		 */
		const bool ifCheckReq() const
		{
			return _checkReq;
		}
		/**
		 ...
		 *
		 */
		bool checkReq(unsigned char req);
	...
	private:
		...
		const bool _checkReq;							/**< 是否检查请求序号*/
		unsigned char _req;								/**< 请求序号*/
};
