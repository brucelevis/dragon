...
unsigned int defaultMemHashCode(const void *anyArg)
{
	...
	DWORD seed = Fir::_mapRedisConfig.size()-1;
	if (0 == seed)
	{
		return 0;
	}
...
bool zMemDBPool::init(hashHandleFunc hashfunc)
{
	...
	for (auto itr=Fir::_mapRedisConfig.begin(); itr!=Fir::_mapRedisConfig.end(); itr++)
	{
		const Fir::stRedisItem & item = itr->second;
		Fir::logger->info("[memdb],hashcode:%d ip:%s port:%d", (DWORD)item.id, item.ip.c_str(), item.port);

		this->putConnInfo(item.id,item.ip,item.port);
	}
...
{//id以后用来进行二级散列时用,例如CHARID,或ACCID，UUID等,0为全局redis
	...
	if (handles.count(tid)==0)
	{
		hashHandlesMap hashHandles;
		for (unsigned int i = 0; i<infoPool.size(); i++)
		{
			zMemDB* new_conn = FIR_NEW zMemDB(infoPool[i].host, infoPool[i].port, infoPool[i].dbIndex,infoPool[i].hashcode);
			if (!new_conn) return NULL;

			if (!new_conn->init()) {SAFE_DELETE(new_conn); return NULL;}

			hashHandles.insert(hashHandlesMap_pair(infoPool[i].hashcode, new_conn));
		}

		mlock.lock();
		handles.insert(handlesPool_pair(tid, hashHandles));
		mlock.unlock();
	}
	...
	mlock.lock();
	auto iter = handles.find(tid);
	if (iter!=handles.end())
	{
		auto handle_map = handles[tid];
	
		if (handle_map.count(hashcode) !=0)
			ret = handle_map[hashcode];
	}
	mlock.unlock();
