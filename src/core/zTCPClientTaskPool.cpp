				...
				//已经在连接状态，检查测试信号指令
				{
					...
					if (task->checkInterval(now))
					{
						...
						else
						{
							...
							//Fir::logger->debug("客户端发送测试信号");
							if (task->sendCmd(&tNullCmd, sizeof(tNullCmd)))
							{
								Fir::logger->debug("[网络]:%s:%d 发送心跳检测成功", task->ip.c_str(), task->port);
								task->setTick();
							}
						}
					}
