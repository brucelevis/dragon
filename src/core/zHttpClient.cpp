...
bool zHttpClient::httpPostJson(std::string &ret, const std::string &postdata)
{
	ret.clear();
	char errorBuffer[CURL_ERROR_SIZE];
	reset();
	...
	curl_slist* send_header = curl_slist_append(NULL, "Content-Type: application/json");	// 自定义头
	//设置本次操作为POST
	curl_easy_setopt(m_curl, CURLOPT_POST, 1);								 
	curl_easy_setopt(m_curl, CURLOPT_CUSTOMREQUEST, "POST");				// 自定义头时, 必须设置该选项
	curl_easy_setopt(m_curl, CURLOPT_ERRORBUFFER, errorBuffer);
	curl_easy_setopt(m_curl, CURLOPT_WRITEFUNCTION, zHttpClient::writer);
	curl_easy_setopt(m_curl, CURLOPT_WRITEDATA, &ret);
	if(postdata.length() != 0)
	{
		if (send_header)
		{
			curl_easy_setopt(m_curl, CURLOPT_HTTPHEADER, send_header);		// 添加自定义头, 表示发送JSON数据
		}
		curl_easy_setopt(m_curl, CURLOPT_POSTFIELDS, postdata.c_str());
	}
	curl_easy_setopt(m_curl, CURLOPT_TIMEOUT, 30);
	...
	Fir::logger->debug("ret= %s", ret.c_str());
	return true;
}
