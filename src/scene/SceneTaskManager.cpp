 ...
 * \return 广播是否成功
 */
bool SceneTaskManager::broadcastByID(const WORD wdServerID, const void *pstrCmd, int nCmdLen)
{
	bool retval = true;
	SceneTaskHashmap_iterator it;
	rwlock.rdlock();
...
bool SceneTaskManager::sendCmdToWorld(DWORD except_charid, const void *pstrCmd, const unsigned int nCmdLen)
{
	rwlock.rdlock();
	for (auto it=sceneTaskSet.begin(); it != sceneTaskSet.end() ; it ++)
	{
		SceneTask* task=it->second;
		if (NULL != task)
		{
			task->sendCmdToWorld(except_charid, pstrCmd, nCmdLen);
		}
	}
	rwlock.unlock();
