...
void ServerManager::sendAllGatewayToMe(ServerTask *task)
{
	...
	CMD::SUPER::t_Startup_ServerEntry_NotifyMe *ptCmd = (CMD::SUPER::t_Startup_ServerEntry_NotifyMe *)pBuffer; 
	...
	if (ptCmd->size)
	{
		task->sendCmd(ptCmd, sizeof(CMD::SUPER::t_Startup_ServerEntry_NotifyMe) + ptCmd->size * sizeof(CMD::SUPER::ServerEntry));
	}
}
...
void ServerManager::notifyGatewayReconnectScene(ServerTask *task)
{
	if (task->getType() == SCENESSERVER)
	{
		...
		if (it == sceneinfo.end())
		{
			...
			broadcastByType(GATEWAYSERVER, &send, sizeof(send));
		}
		else
		{
			// 动态更换IP重新连接,断开旧的
			...
			{
				CMD::SUPER::t_GateTerminateConnect s;
				strncpy(s.pstrIP, it->second.c_str(), MAX_IP_LENGTH - 1);
				s.port = task->getWdPort();
				broadcastByType(GATEWAYSERVER, &s, sizeof(s));
