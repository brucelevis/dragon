#ifndef _seFLCmdHandler_h_
#define _seFLCmdHandler_h_

#include "Fir.h"
#include "proto/fl_session_protocol.pb.h"

class seFLCmdHandler : public Singleton<seFLCmdHandler>
{
	friend class Singleton<seFLCmdHandler>;
	public:
		...
		template<typename T>
		bool sendCmdToFL(fl_session::MSG_ID msgid, const T& msg)
		{
			std::string content;
	...
	protected:
		...
		typedef std::map<fl_session::MSG_ID, HANDLE_FUNC>      HANDLE_FUNC_MAP;
		HANDLE_FUNC_MAP   m_HandleFuncMap;  
		void __RegisterHandleFunc (fl_session::MSG_ID msgid, HANDLE_FUNC func);
};
